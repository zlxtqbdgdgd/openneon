...
/* feat-014: per-relation LFC stats, one row per relation seen by the LFC. */
typedef struct LfcPerRelStatsRec
{
	Oid			relfilenode;
	Oid			reltablespace;
	Oid			reldatabase;
	int64		pages_in_cache;
	uint64		hits;
	uint64		misses;
	uint64		evictions;
	TimestampTz last_access;	/* 0 if never accessed */
} LfcPerRelStatsRec;
extern LfcPerRelStatsRec *lfc_get_per_relation_stats(size_t *num_entries);
